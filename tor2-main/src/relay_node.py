...
class RelayServicer(tor_pb2_grpc.RelayServicer):
    ...
    def BackwardMessage(self, request, context):
        # Accept both bytes and str for session_id
        if isinstance(request.session_id, bytes):
            sid = request.session_id.decode('utf-8')
        else:
            sid = request.session_id
        return_address = self.return_addresses[sid]
        ...
        if isinstance(request.session_id, bytes):
            ...
            sid = request.session_id
        encrypted_key = rsa_encrypt(self.session_keys[sid], aes_key)
    ...
    def ForwardMessage(self, request, context):
        ...
        if isinstance(request.session_id, bytes):
            ...
            sid = request.session_id
        self.return_addresses[sid] = client_address
        ...
        if self.relay_type == self.EXIT_NODE:
            ...
            # Accept both bytes and str for destination
            if isinstance(inner_dict['destination'], bytes):
                dest = inner_dict['destination'].decode('utf-8')
            else:
                dest = inner_dict['destination']
            headers, content = get_page(dest)
            ...
            inner_aes_encrypted_key = rsa_encrypt(
                self.session_keys[sid], inner_aes_key)
            ...
            if isinstance(request.session_id, bytes):
                ...
                sid = request.session_id
            print("DEBUG: Sending back to previous relay: ", self.return_addresses[sid])
            ...
            if isinstance(request.session_id, bytes):
                ...
                sid = request.session_id
            stub = tor_pb2_grpc.RelayStub(grpc.insecure_channel(self.return_addresses[sid]))
            stub.BackwardMessage(msg)
