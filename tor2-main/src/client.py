...
class JTor_Client(cmd.Cmd):
    ...
    def do_help(self, arg):
        if arg.lower() == 'send':
            print("Send a message through the network.")
            print("Usage: send <message>")
            print("Example: send Hello")
        elif arg.lower() == 'exit':
            ...
            print("  send: Send a message through the network.")
    ...
    def send_message(self, message):
        ...
        self.response_received = False
        # inner layer: payload and destination
        inner_aes_key = os.urandom(32)
        inner_msg = pickle.dumps({
            "message": message.encode('utf-8'),
            "destination": self.dest_addr.encode('utf-8'),
        })
        ...
        if not self.response_received:
            ...
            if self.current_retries < self.num_retries:
                # get new relay nodes
                self.get_relay_nodes()
                self.build_circuit()
                self.send_message(message)
